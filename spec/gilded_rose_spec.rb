...
itmes_test = [
  ...
  Item.new('Backstage passes to a TAFKAL80ETC concert', 5, 9),
...
describe GildedRose do
  describe '#update_quality' do
    it 'Expiry date expired and angy brie' do
      expect(verify_proof(normal)).to eq('foo, -1, 6, faa, -3, 0, ')
    end
    it 'Aged Brie' do
      expect(verify_proof(aged_brie)).to eq('Aged Brie, 9, 11, Aged Brie, 4, 50, ')
    end
    it 'Sulfuras' do
      expect(verify_proof(sulfuras)).to eq('Sulfuras, Hand of Ragnaros, 0, 80, Sulfuras, Hand of Ragnaros, -1, 80, ')
    end
    it 'Backstage passes' do
      expect(verify_proof(backstage)).to eq('Backstage passes to a TAFKAL80ETC concert, 8, 12, Backstage passes to a TAFKAL80ETC concert, 4, 12, Backstage passes to a TAFKAL80ETC concert, -2, 0, ')
    end
    it 'Conjured' do
